"""Extração e importação TX8. Originais protegidos; importações salvas em cópias."""
...
class Tx8App(tk.Tk):
    def __init__(self):
        super().__init__()
        self.title('Koudelka TX8 — Extrator / Importador v2')
    ...
    def _build(self):
        ...
        for text, command in [('Abrir TX8…', self.open_file), ('Abrir pasta…', self.open_folder),
                              ('Recarregar', self.reload_current), ('Importar PNG…', self.open_importer)]:
            ttk.Button(toolbar, text=text, command=command).pack(side='left', padx=(0, 5))
        ttk.Label(toolbar, text='Originais protegidos').pack(side='right')
    ...
    def open_importer(self):
        ...
        if self.import_window is not None and self.import_window.winfo_exists():
            ...
            return
        if self.current is None:
            messagebox.showinfo('Importar PNG', 'Abra o TX8 original e selecione uma imagem primeiro.', parent=self)
            return
        from koudelka_tx8_import_gui import ImportWindow
        source, selected = self.current
        self.import_window = ImportWindow(self, source, selected)
    ...
    def close(self):
        if self.import_window is not None and self.import_window.winfo_exists():
            ...
            if self.import_window.winfo_exists():
                return
        if self.busy:
