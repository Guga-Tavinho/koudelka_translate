...
def match_png_batch(source: Tx8File, paths: Iterable[Path | str]) -> dict[int, Path]:
    """Associa por número inicial (base 1), nunca pela ordem do seletor de arquivos."""
    matched = {}
    for value in paths:
        path = Path(value).resolve()
        match = re.match(r'^([0-9]+)(?:[_. -]|$)', path.stem)
        if path.suffix.lower() != '.png' or match is None:
            raise ValueError(f'{path.name}: use um nome iniciado pelo número da imagem, '
                             'como 001_tela_320x240_PTBR.png. '
                             'Para nomes diferentes, use a importação individual.')
        number = int(match.group(1))
        if not 1 <= number <= len(source.textures):
            raise ValueError(f'{path.name}: imagem {number:03d} fora do intervalo '
                             f'001 a {len(source.textures):03d} deste TX8.')
        index = number - 1
        if index in matched:
            raise ValueError(f'Dois PNGs apontam para a imagem {number:03d}:\n'
                             f'{matched[index].name}\n{path.name}\n'
                             'Selecione só uma versão por imagem (linear, painéis ou tela).')
        if not path.is_file():
            raise ValueError(f'PNG não encontrado: {path}')
        matched[index] = path
    if not matched:
        raise ValueError('Selecione pelo menos um PNG.')
    return dict(sorted(matched.items()))


def prepare_import_batch(source: Tx8File, paths: Iterable[Path | str]) -> dict[int, PreparedImport]:
    """Prepara tudo antes de retornar; falhas não aplicam um lote pela metade."""
    prepared = {}
    for number, path in match_png_batch(source, paths).items():
        try:
            prepared[number] = prepare_import(source.textures[number], path)
        except Exception as error:
            raise ValueError(f'Imagem {number + 1:03d} — {path.name}:\n{error}\n\n'
                             'Nenhuma imagem deste lote foi aplicada. '
                             'As importações anteriores foram mantidas.') from error
    return prepared
