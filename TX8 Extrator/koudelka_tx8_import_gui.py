...
class ImportWindow(tk.Toplevel):
    def __init__(self, master, source, selected=0):
        ...
        navigation = ttk.Frame(choose)
        navigation.grid(row=0, column=2, padx=(8, 0))
        self.previous_button = ttk.Button(navigation, text='◀ Anterior',
                                          command=lambda: self.move_image(-1))
        self.previous_button.pack(side='left')
        self.image_counter = ttk.Label(navigation, width=8, anchor='center')
        self.image_counter.pack(side='left', padx=3)
        self.next_button = ttk.Button(navigation, text='Próxima ▶',
                                      command=lambda: self.move_image(1))
    ...
    def update_navigation(self):
        count = len(self.source.textures)
        self.image_counter.configure(text=f'{self.selected + 1} / {count}')
        self.previous_button.configure(state='normal' if not self.busy and self.selected > 0 else 'disabled')
        self.next_button.configure(state='normal' if not self.busy and self.selected < count - 1 else 'disabled')

    def move_image(self, direction):
        if self.busy:
            return
        number = max(0, min(len(self.source.textures) - 1, self.selected + direction))
        if number == self.selected:
            return
        self.selected = number
        self.selector.current(number)
        self.set_view_for_selected()
