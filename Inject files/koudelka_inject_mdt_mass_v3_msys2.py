...
def copy_companion_cue(original_bin: Path, target_bin: Path) -> Path | None:
    ...
    for encoding in ("utf-8-sig", "cp1252", "latin-1"):
        try:
            ...
            # utf-8-sig is useful for reading because it also removes a BOM,
            # but writing with it would add EF BB BF before FILE. psxinject
            # treats that marker as part of the keyword and rejects the CUE.
    ...
    # Write bytes directly so Windows does not turn existing CRLF sequences
    # into CRCRLF. CUE files consumed by psxinject must start exactly at FILE.
...
class InjectorGUI(tk.Tk):
    ...
    def _build_ui(self) -> None:
        ...
        source_frame = ttk.LabelFrame(container, text="1. Arquivos MDT", padding=10)
        ...
        internal_row.pack(fill="x", pady=(6, 0))
        ttk.Label(internal_row, text="Caminho interno:", width=17).pack(side="left")
        ...
        bin_frame = ttk.LabelFrame(container, text="2. Imagem do jogo", padding=10)
        ...
        tools_frame = ttk.LabelFrame(container, text="3. Ferramentas e opções", padding=10)
        ...
        small_row.pack(fill="x", pady=(6, 0))
        ttk.Label(small_row, text="Prefixo no CD:", width=17).pack(side="left")
        ...
        preview_frame = ttk.LabelFrame(container, text="Arquivos que serão injetados", padding=8)
        ...
        log_frame = ttk.LabelFrame(container, text="Progresso", padding=8)
        ...
        ttk.Label(action_row, textvariable=self.status_var, width=28).pack(side="left", padx=10)
