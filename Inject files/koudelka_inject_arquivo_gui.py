...
TRANSLATION_SUFFIX = re.compile(
    r"(?i)(?:_TRADUZID[OA]|_IMPORTAD[OA]|_PTBR|_PT_BR|_REVISAD[OA]|_FINAL)"
    r"(?:_[A-Z0-9]+)*$"
)
...
class GenericInjectorGUI(tk.Tk):
    ...
    def _build_ui(self) -> None:
        ...
        # Configuracoes em abas reduzem a altura minima da janela. A barra de
        # acao e empacotada no rodape antes da area expansivel, garantindo que
        # o botao de injecao continue visivel em resolucoes menores.
        settings_tabs = ttk.Notebook(main)
        settings_tabs.pack(fill="x")
        add_frame = ttk.Frame(settings_tabs, padding=7)
        bin_frame = ttk.Frame(settings_tabs, padding=7)
        tools = ttk.Frame(settings_tabs, padding=7)
        settings_tabs.add(add_frame, text="1. Adicionar arquivo")
        settings_tabs.add(bin_frame, text="3. Imagem do jogo")
        settings_tabs.add(tools, text="4. Ferramentas e opções")
        ...
        ttk.Label(internal_row, text="Caminho no CD:", width=15).pack(side="left")
        ...
        ttk.Label(log_row, text="Log CSV:", width=15).pack(side="left")
        ...
        ttk.Label(actions, textvariable=self.status_var, width=20).pack(
            side="left", padx=7
        )
        ...
        work_tabs = ttk.Notebook(main)
        work_tabs.pack(fill="both", expand=True, pady=(6, 0))
        queue_frame = ttk.Frame(work_tabs, padding=6)
        progress_frame = ttk.Frame(work_tabs, padding=6)
        work_tabs.add(queue_frame, text="2. Fila de injeção")
        work_tabs.add(progress_frame, text="Progresso / log")

        top_queue = ttk.Frame(queue_frame)
        top_queue.pack(fill="x", pady=(0, 6))
        ttk.Label(queue_frame, textvariable=self.summary_var).pack(anchor="w", pady=(0, 4))
        ...
        self.tree = ttk.Treeview(
            tree_holder, columns=columns, show="headings", selectmode="browse", height=6
        )
