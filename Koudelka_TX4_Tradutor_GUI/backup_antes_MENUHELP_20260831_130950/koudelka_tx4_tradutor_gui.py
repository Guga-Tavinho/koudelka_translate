...
Editor gráfico para MENU/ITEMS/001.TX4 da versão americana de Koudelka.

Regras preservadas do fluxo final aprovado:
...
APP_TITLE = "Koudelka TX4 Tradutor — Arial"
WIDTH = 256
STORED_HEIGHT = 144
VIEW_HEIGHT = 128
...
def measure_translation(
    ...
    lines = wrap_text(draw, font, text, TEXT_MAX_X - TEXT_X)
...
def draw_ttf_to_indices(
    ...
    lines, _widest = measure_translation(text, font)
    if len(lines) > max_lines:
        raise ValueError(
            f"usa {len(lines)} linhas; o máximo para este bloco é {max_lines}"
        )

    mask = Image.new("1", (WIDTH, VIEW_HEIGHT), 0)
    draw = ImageDraw.Draw(mask)
    for number, line in enumerate(lines):
        line_y = y + number * line_height
        if line_y >= VIEW_HEIGHT:
            raise ValueError("ultrapassa a altura de 128 pixels")
        draw.text((TEXT_X, line_y), line, font=font, fill=1)

    pixels = mask.load()

    shadow_points: list[tuple[int, int]] = []
    for py in range(VIEW_HEIGHT):
        for px in range(WIDTH):
            if pixels[px, py]:
                sx, sy = px + 1, py + 1
                if sx < WIDTH and sy < VIEW_HEIGHT:
...
def indices_to_rgb(indices: list[int]) -> Image.Image:
    image = Image.new("RGB", (WIDTH, VIEW_HEIGHT), (0, 0, 0))
    image.putdata([DISPLAY_PALETTE[value & 0x0F] for value in indices])
    return image


def indices_to_ocr_image(indices: list[int], crop_top: int, crop_bottom: int) -> Image.Image:
...
class TX4Project:
    ...
    def block_count(self) -> int:
        return len(self.data) // BLOCK_SIZE if self.data else 0

    def load_tx4(self, path: Path) -> None:
        data = bytearray(path.read_bytes())
        if not data or len(data) % BLOCK_SIZE:
            raise ValueError(
                f"Tamanho inválido: {len(data)} bytes. O arquivo deve ser múltiplo de {BLOCK_SIZE}."
            )
        self.tx4_path = path
        self.data = data
        names = self._load_names_from_neighbor(path.parent / "ITMTBL.ITM")
        self.entries = [
    ...
    def block_indices(self, block: int) -> list[int]:
        ...
        raw = bytes(self.data[start + IMAGE_DATA_OFFSET : start + BLOCK_SIZE])
        indices, _height = decode_4bpp(raw)
        visible = indices[: WIDTH * VIEW_HEIGHT]
        if len(visible) != WIDTH * VIEW_HEIGHT:
            raise ValueError(f"Bloco {block} não contém a área visível esperada.")
        return visible

    def original_image(self, block: int) -> Image.Image:
        return indices_to_rgb(self.block_indices(block))
    ...
    def generate(self, output: Path, font: ImageFont.FreeTypeFont) -> tuple[int, int]:
        ...
        for entry in self.entries:
            ...
            start = (entry.block - 1) * BLOCK_SIZE
            block = bytes(self.data[start : start + BLOCK_SIZE])
            new_block = bytearray(block)
            new_block[IMAGE_DATA_OFFSET : IMAGE_DATA_OFFSET + len(encoded)] = encoded
            new_block[:PROTECTED_BYTES] = block[:PROTECTED_BYTES]
            rebuilt[start : start + BLOCK_SIZE] = new_block
            changed += 1
...
class TranslatorGUI:
    ...
    def _build_ui(self) -> None:
        ...
        self.original_label = ttk.Label(original_frame, anchor=tk.CENTER)
        ...
        self.original_text = tk.Text(right, height=7, wrap=tk.WORD, undo=True)
        ...
        self.translation_text = tk.Text(right, height=10, wrap=tk.WORD, undo=True)
    ...
    def open_tx4(self, path: Path | None = None) -> None:
        ...
        if path is None:
            selected = filedialog.askopenfilename(
                title="Abrir MENU/ITEMS/001.TX4",
        ...
        self.refresh_tree()
        if self.project.block_count != EXPECTED_BLOCKS:
    ...
    def choose_font(self) -> None:
        ...
        self.rules_label.configure(
            text=f"{self.font_path.name} 11 px • altura 15 • itens: 6 linhas • documentos: 8 linhas"
        )
        self.update_preview()

    def import_csv(self, path: Path | None = None) -> None:
        if not self.project.data:
            messagebox.showinfo("Abra o TX4", "Abra primeiro o arquivo 001.TX4.")
            return
        self.apply_current()
        if path is None:
            selected = filedialog.askopenfilename(
    ...
    def save_csv(self, save_as: bool = False) -> None:
        if not self.project.data:
            messagebox.showinfo("Nada para salvar", "Abra primeiro o arquivo 001.TX4.")
            return
        self.apply_current()
        path = None if save_as else self.csv_path
        if path is None:
            default_name = "koudelka_230_TRADUCAO_GUI.csv"
    ...
    def load_entry(self, block: int) -> None:
        ...
        self.type_label.configure(text="DOCUMENTO (8 linhas)" if entry.is_document else "ITEM (6 linhas)")
    ...
    def update_images(self) -> None:
        if self.current_block is None:
            return
        original = self.project.original_image(self.current_block)
        display_original = original.resize((WIDTH * 2, VIEW_HEIGHT * 2), Image.Resampling.NEAREST)
    ...
    def update_preview(self) -> None:
        ...
        try:
            ...
            lines, widest = measure_translation(translation, self.font) if translation else ([], 0)
            max_lines = DOCUMENT_MAX_LINES if temporary.is_document else ITEM_MAX_LINES
            if translation:
        ...
        display_preview = preview.resize((WIDTH * 2, VIEW_HEIGHT * 2), Image.Resampling.NEAREST)
        self.preview_photo = ImageTk.PhotoImage(display_preview)
    ...
    def ocr_all(self) -> None:
        ...
        if not messagebox.askyesno(
            ...
            "O OCR dos 230 blocos pode demorar vários minutos. Deseja continuar?",
        ):
            return
        self.apply_current()
        self.ocr_running = True
