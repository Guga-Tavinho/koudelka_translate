...
class ItemRegressionTests(unittest.TestCase):
    def test_all_item_groups_wrap_in_left_column_but_documents_keep_full_width(self):
        project = tx4.TX4Project()
        project.load_tx4(ITEMS)
        font = tx4.load_font(tx4.DEFAULT_ARIAL)

        whiskey = project.entry(11)
        whiskey.translation = (
            'Garrafa de uísque escocês. Seu alto teor alcoólico pode despertar '
            'inconscientes. Revive personagens nocauteados e restaura um pouco de HP.'
        )
        pixels, lines = project.rendered_indices(whiskey, font)
        measured, widest = project.measure(whiskey.translation, font, whiskey)
        self.assertEqual(lines, measured)
        self.assertEqual(len(lines), 5)
        self.assertLessEqual(widest, tx4.ITEM_TEXT_MAX_WIDTH)
        for y in range(project.text_top(whiskey.block), tx4.VIEW_HEIGHT):
            black_x = [x for x, value in enumerate(pixels[y*tx4.WIDTH:(y+1)*tx4.WIDTH])
                       if value == tx4.COLOR_MAIN]
            if black_x:
                self.assertLess(max(black_x), tx4.ITEM_TEXT_MAX_X)

        daniels_arm = project.entry(219)
        self.assertTrue(daniels_arm.uses_item_column)
        daniels_arm.translation = (
            'Braço mumificado de Daniel, escondido dentro de sua estátua. '
            'Uma relíquia sagrada de grande poder.'
        )
        pixels, lines = project.rendered_indices(daniels_arm, font)
        measured, widest = project.measure(daniels_arm.translation, font, daniels_arm)
        self.assertEqual(lines, measured)
        self.assertEqual(len(lines), 4)
        self.assertLessEqual(widest, tx4.ITEM_TEXT_MAX_WIDTH)
        for y in range(project.text_top(daniels_arm.block), tx4.VIEW_HEIGHT):
            black_x = [x for x, value in enumerate(pixels[y*tx4.WIDTH:(y+1)*tx4.WIDTH])
                       if value == tx4.COLOR_MAIN]
            if black_x:
                self.assertLess(max(black_x), tx4.ITEM_TEXT_MAX_X)

        document = project.entry(116)
        self.assertTrue(document.is_document)
        self.assertFalse(document.uses_item_column)
        document.translation = 'Documentos usam a largura completa.'
        doc_lines, doc_width = project.measure(document.translation, font, document)
        self.assertEqual(len(doc_lines), 1)
        self.assertGreater(doc_width, tx4.ITEM_TEXT_MAX_WIDTH)
        self.assertLessEqual(doc_width, tx4.TEXT_MAX_X - tx4.TEXT_X)

        titled_document = project.entry(201)
        self.assertFalse(titled_document.is_document)
        self.assertFalse(titled_document.uses_item_column)
        titled_document.translation = 'Páginas de cartas com título usam a largura completa.'
        _lines, titled_width = project.measure(titled_document.translation, font, titled_document)
        self.assertGreater(titled_width, tx4.ITEM_TEXT_MAX_WIDTH)

    def test_document_starting_with_accent_is_not_rejected_or_clipped(self):
        project = tx4.TX4Project()
        project.load_tx4(ITEMS)
        font = tx4.load_font(tx4.DEFAULT_ARIAL)
        document = project.entry(116)
        document.translation = 'É possível preservar o acento na primeira linha.'
        pixels, lines = project.rendered_indices(document, font)
        self.assertTrue(lines)
        self.assertFalse(project.validate(font))
        self.assertIn(tx4.COLOR_MAIN, pixels[:15*tx4.WIDTH])
    ...
    def test_sa_pistol_keeps_black_glyphs_inside_left_column(self):
        ...
        actual, lines = p.rendered_indices(p.entry(79),tx4.load_font(tx4.DEFAULT_ARIAL))
        expected = p.block_indices(79)
        self.assertEqual(actual[:7680],expected[:7680])
        self.assertEqual(set(actual[7680:]),{0,1})
        self.assertGreater(len(lines), 2)  # As linhas manuais longas tambem sao ajustadas.
        for y in range(p.text_top(79), tx4.VIEW_HEIGHT):
            black_x = [x for x, value in enumerate(actual[y*tx4.WIDTH:(y+1)*tx4.WIDTH])
                       if value == tx4.COLOR_MAIN]
            if black_x:
                self.assertLess(max(black_x), tx4.ITEM_TEXT_MAX_X)
