...
- blocos com título: mantém o título original e redesenha apenas a descrição;
- páginas de continuação: limpa toda a área textual e escreve até 8 linhas;
- Arial 11 px, line-height 15, preto nativo sem antialias e SEM sombra;
...
TEXT_X = 10
TEXT_MAX_X = 252
# Os sprites dos itens sao compostos pelo jogo sobre a metade direita da tela.
# A versao anterior usava TEXT_MAX_X em alguns blocos de itens, portanto uma
# traducao longa podia ser desenhada por baixo do sprite. O original prefere
# mais linhas na coluna esquerda em todos os blocos que exibem um item.
ITEM_TEXT_MAX_X = 170
ITEM_TEXT_MAX_WIDTH = ITEM_TEXT_MAX_X - TEXT_X
ITEM_BODY_Y = 30
COLOR_MAIN = 1  # Preto nativo dos títulos (CLUT 0x8000); zero é transparente.
COLOR_SHADOW = None  # Itens: somente a máscara das letras, sem deslocamento.
ITEM_LAST_BLOCK = 114
EXTRA_TITLED_BLOCKS = frozenset({115, 154, 201, 206, 211, 215, 216, 217,
                               218, 219, 220, 221, 222, 224, 225, 226,
                               227, 228, 229, 230})
# Itens adicionais encontrados depois das páginas de documentos. Eles têm
# título próprio e também recebem um ícone na metade direita durante o jogo.
# O bloco 223 é continuação, por isso não entra nesta lista.
EXTRA_ITEM_BLOCKS = frozenset({215, 216, 217, 218, 219, 220, 221, 222,
...
# Fallback dos helpers: CLUT original dos itens, nunca a antiga paleta laranja
# ilustrativa. A interface usa sempre a paleta lida do próprio bloco.
DISPLAY_PALETTE = [
    tuple((((word >> shift) & 31) << 3) | (((word >> shift) & 31) >> 2)
          for shift in (0, 5, 10))
    for word in (0, 32768, 46643, 46679, 50871, 45622, 44566, 44533,
                 0, 41396, 46643, 41363, 40306, 54042, 58270, 65535)
]
...
class Entry:
    ...
    def is_document(self) -> bool:
        return self.block > ITEM_LAST_BLOCK and self.block not in EXTRA_TITLED_BLOCKS

    @property
    def uses_item_column(self) -> bool:
        """Verdadeiro quando o jogo compõe um ícone à direita da descrição."""
        return self.block <= ITEM_LAST_BLOCK or self.block in EXTRA_ITEM_BLOCKS
...
def draw_ttf_to_indices(
    ...
    color_shadow: int | None = COLOR_SHADOW,
) -> list[str]:
    ...
    # Algumas letras acentuadas da Arial (por exemplo, É e ã) ultrapassam em
    # 1 px a origem vertical informada pelo Pillow. Documentos começam em Y=0;
    # desloque o conjunto somente quando necessário para preservar o acento em
    # vez de rejeitar ou recortar a primeira linha.
    vertical_shift = 0
    for number, line in enumerate(lines):
        ...
        _left, top, _right, _bottom = draw.textbbox((x, line_y), line, font=font)
        vertical_shift = max(vertical_shift, -top)

    for number, line in enumerate(lines):
        line_y = y + vertical_shift + number * line_height
        x = (WIDTH - text_width(draw, font, line)) // 2 if centered else TEXT_X
        if line:
            left, top, right, bottom = draw.textbbox((x, line_y), line, font=font)
            margin = 1 if color_shadow is not None else 0
            if left < 0 or top < 0 or right + margin > WIDTH or bottom + margin > height:
                detail = ' (incluindo sombra)' if margin else ''
                raise ValueError(f"linha {number + 1} ultrapassa a área de {WIDTH}x{height} pixels{detail}")
        if line_y >= height:
    ...
    # MENUHELP continua passando explicitamente seu índice de sombra cinza.
            ...
            for px in range(WIDTH):
                if pixels[px, py]:
                    ...
                    if sx < WIDTH and sy < height:
                        indices[sy * WIDTH + sx] = color_shadow
...
class TX4Project:
    ...
    def max_lines(self, entry: Entry) -> int:
        if self.is_menuhelp:
            return HELP_MAX_LINES
        if entry.is_document:
            return DOCUMENT_MAX_LINES
        return 7 if self.text_top(entry.block) == TITLE_HEIGHT else ITEM_MAX_LINES

    def text_top(self, block: int) -> int:
        if self.is_menuhelp or self.entry(block).is_document:
            return 0
        # Mantém o layout de 30px dos itens normais. Alguns títulos adicionais
        # têm descrição já a partir de 18px; não apagar nem sobrepor essa faixa.
        if block in EXTRA_TITLED_BLOCKS:
            indices = self.block_indices(block)
            if any(indices[TITLE_HEIGHT * WIDTH:ITEM_BODY_Y * WIDTH]):
                return TITLE_HEIGHT
        return ITEM_BODY_Y

    def text_width_limit(self, entry: Entry | None = None) -> int:
        if self.is_menuhelp:
            return HELP_MAX_WIDTH
        if entry is not None and entry.uses_item_column:
            return ITEM_TEXT_MAX_WIDTH
        return TEXT_MAX_X - TEXT_X
    ...
    def measure(
        ...
        return measure_translation(text, font, self.text_width_limit(entry))
    ...
    def rendered_indices(
        ...
        if self.is_menuhelp:
            ...
            palette = self.blocks[entry.block - 1].palette
            if palette[0] != 0 or palette[COLOR_MAIN] != 0x8000:
                raise ValueError('Paleta de itens sem o preto nativo esperado no índice 1. '
                                 'Use o TX4 original ou a versão preta sem sombra; '
                                 'a paleta não será alterada automaticamente.')
            top = self.text_top(entry.block)
            for py in range(top, VIEW_HEIGHT):
                base = py * WIDTH
                for px in range(WIDTH):
                    indices[base + px] = 0
            lines = draw_ttf_to_indices(
                ...
                y=top,
                font=font,
                max_lines=self.max_lines(entry),
                max_width=self.text_width_limit(entry),
    ...
    def ocr_block(self, block: int) -> tuple[str, str]:
        ...
        if entry.is_document:
            ...
            return "", run_tesseract(full, 6)
        title = indices_to_ocr_image(indices, 0, TITLE_HEIGHT)
        description = indices_to_ocr_image(indices, TITLE_HEIGHT, VIEW_HEIGHT)
...
class TranslatorGUI:
    ...
    def _build_ui(self) -> None:
        ...
        self.rule_note = ttk.Label(
            right,
            text=(
                "Todos os blocos de itens, inclusive os adicionais 215–230, mantêm a "
                "descrição na coluna esquerda, sem avançar sobre a imagem. "
                "Páginas de documentos continuam usando a largura total."
            ),
            wraplength=440,
            foreground="#555555",
        )
    ...
    def update_rules(self):
        if self.project.is_menuhelp:
            ...
            self.rules_label.configure(text=f'{self.font_path.name} 11 px • preto sem sombra • títulos: 6/7 linhas • continuações: 8 linhas')
            self.rule_note.configure(text='Títulos preservados nos blocos 1–114 e nos blocos adicionais com título próprio. '
                                     'Nas continuações, traduza a página inteira. Preto nativo, sem sombra; paleta preservada. '
                                     'Blocos sem texto PT-BR permanecem como estão no arquivo aberto.')
    ...
    def load_entry(self, block: int) -> None:
        ...
        self.type_label.configure(text='AJUDA (14 linhas)' if self.project.is_menuhelp
                                   else ("CONTINUAÇÃO (8 linhas)" if entry.is_document
                                         else f"TÍTULO PRESERVADO ({self.project.max_lines(entry)} linhas)"))
        ...
        style = '' if self.project.is_menuhelp else ' • sem sombra'
        color_text = (f'Fonte para gerar: preta • sem sombra • preto nativo dos títulos'
                      if not self.project.is_menuhelp and word == 0x8000
                      else f'Fonte para gerar: {native}{style} • {flag}')
        self.color_label.configure(text=color_text)
    ...
    def update_preview(self) -> None:
        ...
        try:
            ...
            lines, widest = self.project.measure(translation, self.font, temporary) if translation else ([], 0)
