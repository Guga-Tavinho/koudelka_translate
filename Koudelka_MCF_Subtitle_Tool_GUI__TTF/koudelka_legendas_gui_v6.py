...
class EngineConfig:
    ...
    # "crt" rasteriza diretamente na grade final, sem reamostragem: isso
    # mantém os caracteres finos e separados mesmo após a filtragem da TV.
    # Em todos os modos TTF somente o índice 1 (branco opaco) é gravado.
    render_mode: str = "crt"
...
class KoudelkaSubtitleEngine:
    def __init__(self, config: Optional[EngineConfig] = None):
        ...
        self._ttf_line_cache: dict[tuple[str, str, str, int], np.ndarray] = {}

    @property
    def font_ready(self) -> bool:
        if self.cfg.render_mode in {"crt", "ttf"}:
    ...
    def _compose_line_crt(self, text: str) -> np.ndarray:
        """
        Rasteriza a frase diretamente nos pixels finais do MCF.

        A Tahoma é desenhada em modo monocromático diretamente na grade
        nativa, sem uma imagem intermediária ampliada. Não há antialiasing,
        superamostragem, LANCZOS, tons de cinza, contorno ou dilatação: o MCF
        recebe somente fundo 0 e branco opaco no índice 1.
        """
        if self.ttf_path is None:
            raise RuntimeError("Carregue uma fonte TTF/OTF primeiro.")

        key = ("crt-mono", text, str(self.ttf_path), self.cfg.ttf_font_size)
        ...
        font = ImageFont.truetype(
            str(self.ttf_path),
            self.cfg.ttf_font_size,
        )
        left, top, right, bottom = font.getbbox(text, anchor="ls")
        # A Tahoma 12 usa até 12 px acima e 3 px abaixo da linha de base.
        # Baseline 12 acomoda simultaneamente maiúsculas acentuadas e
        # descendentes sem cortar ou deslocar caracteres entre frases.
        baseline = 12
        glyph_top = baseline + top
        glyph_bottom = baseline + bottom
        if glyph_top < 0 or glyph_bottom > self.cfg.glyph_height:
            raise ValueError(
                f"A fonte no tamanho {self.cfg.ttf_font_size} não cabe na faixa "
                f"vertical de {self.cfg.glyph_height}px. Use um tamanho menor."
            )

        margin = 2
        width = max(1, int(right - left) + margin * 2)
        mask_image = Image.new(
            "1",
            (width, self.cfg.glyph_height),
            0,
        )
        ...
        result = np.asarray(mask_image, dtype=np.uint8)

        # Retira apenas o excesso externo, conservando 1 px de respiro.
    ...
    def _compose_line_ttf(self, text: str) -> np.ndarray:
        ...
        key = ("ttf", text, str(self.ttf_path), self.cfg.ttf_font_size)
        cached = self._ttf_line_cache.get(key)
        if cached is not None:
            return cached.copy()
    ...
    def compose_line(self, text: str) -> np.ndarray:
        if self.cfg.render_mode == "crt":
            return self._compose_line_crt(text)
...
class KoudelkaGUI(tk.Tk):
    def __init__(self):
        ...
        self.render_mode_var = tk.StringVar(
            value="Texto CRT nítido (recomendado)"
        )
        ...
        bundled_ttf = Path(__file__).with_name("tahoma.ttf")
        if not bundled_ttf.is_file():
        ...
        self.status_var = tk.StringVar(
            value="Texto CRT nítido pronto. Selecione o MCF e o CSV."
        )
    ...
    def _build_ui(self):
        ...
        render_combo = ttk.Combobox(
            ...
            state="readonly",
            values=(
                "Texto CRT nítido (recomendado)",
                "TTF suavizado (legado)",
                "Fonte nativa FT4",
            ),
            width=30,
        )
    ...
    def _browse_ttf(self):
        ...
        self.render_mode_var.set("Texto CRT nítido (recomendado)")
        self.engine.cfg.render_mode = "crt"
    ...
    def _engine_settings_from_ui(self):
        ...
        render_modes = {
            "Texto CRT nítido (recomendado)": "crt",
            "TTF suavizado (legado)": "ttf",
            "Fonte nativa FT4": "ft4",
        }
        self.engine.cfg.render_mode = render_modes.get(
            self.render_mode_var.get(),
            "crt",
        )

        if self.engine.cfg.render_mode in {"crt", "ttf"}:
