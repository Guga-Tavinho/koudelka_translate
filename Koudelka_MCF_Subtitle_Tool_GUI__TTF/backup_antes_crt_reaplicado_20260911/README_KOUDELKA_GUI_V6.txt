...
A ferramenta possui dois modos de renderização:

- Tahoma Bold (TTF): modo padrão desta pasta. Renderiza a frase inteira
  com superamostragem e kerning. Não cria contorno, halo nem sombra;
  somente as letras são gravadas com o índice 1 (branco opaco) do MCF.
- Fonte nativa FT4: mantém o visual original do jogo.

O arquivo tahomabd.ttf fica junto ao programa e é carregado
automaticamente. Outra fonte .ttf ou .otf pode ser escolhida na interface.
O tamanho recomendado é 11. Valores aceitos: 8 a 16. O antialiasing é
convertido com limiar de 50%, deixando apenas letras brancas limpas.

O MCF continua armazenando bitmaps; o modo TTF melhora a qualidade da
rasterização sem alterar o formato esperado pelo jogo.
